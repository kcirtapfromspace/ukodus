...
impl AntiBot {
    pub fn verify(input: &GameResultInput) -> VerificationResult {
        ...
        if input.avg_move_time_ms < 150 {
            issues.push(format!(
                "avg move time {}ms below minimum 150ms",
                input.avg_move_time_ms
            ));
        }

        // Min single move time: 50ms
        if input.min_move_time_ms < 50 {
            issues.push(format!(
                "min move time {}ms below minimum 50ms",
                input.min_move_time_ms
            ));
        }

        // Min std dev: 100.0
        if input.move_time_std_dev < 100.0 {
            issues.push(format!(
                "move time std dev {:.1} below minimum 100.0",
                input.move_time_std_dev
            ));
        }

        // Expert perfect game check: suspicious if expert+ with 0 mistakes, 0 hints
